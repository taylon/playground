...
typedef enum CursorMode {
  COMPOSITOR_CURSOR_PASSTHROUGH,
  COMPOSITOR_CURSOR_MOVE,
  COMPOSITOR_CURSOR_RESIZE,
} CursorMode;
...
// NOTE(taylon): a view is pretty much a "window" from what I understood so far
typedef struct View {
  Server *server;
...
static void output_frame(struct wl_listener *listener, void *data) {
  ...
  // TODO(taylon): if our hardware supports cursors (which it probably does),
  // just remove this
...
static void focus_view(View *view, struct wlr_surface *surface) {
  if (view == NULL) {
    return;
  }
...
static void begin_interactive(View *view, CursorMode mode, uint32_t edges) {
  Server *server = view->server;
  ...
  if (mode == COMPOSITOR_CURSOR_MOVE) {
...
static void xdg_toplevel_request_move(struct wl_listener *listener,
  ...
  begin_interactive(view, COMPOSITOR_CURSOR_MOVE, 0);
}
...
static void xdg_toplevel_request_resize(struct wl_listener *listener,
  ...
  begin_interactive(view, COMPOSITOR_CURSOR_RESIZE, event->edges);
}
...
static void server_new_xdg_surface(struct wl_listener *listener, void *data) {
  ...
  /* cotd */
  // TODO(taylon): we will most likely be able to remove these and everything
...
// NOTE(taylon): I did not even read through this function properly because I'm
// pretty sure that we will not need this functionality (we will be building
// everything from scratch very soon)
 ...
 */
static bool view_at(View *view, double lx, double ly,
...
static void process_cursor_motion(Server *server, uint32_t time) {
  ...
  if (server->cursor_mode == COMPOSITOR_CURSOR_MOVE) {
    process_cursor_move(server, time);
    return;
  } else if (server->cursor_mode == COMPOSITOR_CURSOR_RESIZE) {
...
// NOTE(taylon): this is another one that we will not need
...
static void server_cursor_button(struct wl_listener *listener, void *data) {
  ...
  if (event->state == WLR_BUTTON_RELEASED) {
    server->cursor_mode = COMPOSITOR_CURSOR_PASSTHROUGH;
  } else {
...
static void server_new_keyboard(Server *server,
                                struct wlr_input_device *device) {
 ...
 * acceleration, etc. */
static void server_new_pointer(Server *server,
                               struct wlr_input_device *device) {
...
// NOTE(taylon): sets the selection of what?
...
int main(int argc, char **argv) {
  ...
  if (!socket) {
    fputs("unable to add unix socket to the Wayland display", stderr);
  ...
  if (!wlr_backend_start(server.backend)) {
    fputs("unable to start the wlroots backend", stderr);
